...
namespace DotNetDetour
{
    ...
    public class MethodHook
    {
        ...
        private List<DestAndOri> destAndOris = new List<DestAndOri>();

        /// <summary>
        /// 安装监视器
        /// </summary>
        public void Install(string dir = null)
        {
            if (installed)
                return;
            installed = true;
            ...
            foreach (var monitor in monitors)
            {
                ...
                // var hookMethods = all.Where(t => t.GetCustomAttributes(typeof(HookMethodAttribute),true).Any(a => typeof(HookMethodAttribute).IsAssignableFrom(HookMethodAttribute)));
                //var originalMethods = all.Where(t => t.GetCustomAttributes(typeof(HookMethodAttribute),true).Any(a => typeof(OriginalMethodAttribute).IsAssignableFrom((HookMethodAttribute)a.AttributeType))).ToArray();
                var hookMethods = all.Where(t => t.GetCustomAttributes(typeof(HookMethodAttribute), true).Any(a => typeof(HookMethodAttribute).IsAssignableFrom(a.GetType())));
                var originalMethods = all.Where(t => t.GetCustomAttributes(typeof(HookMethodAttribute), true).Any(a => typeof(OriginalMethodAttribute).IsAssignableFrom(a.GetType()))).ToArray();
                ...
                foreach (var hookMethod in hookMethods)
                {
                    ...
                    else
                    {
                        ...
                        var originalMethodName = (hookMethod.GetCustomAttributes(typeof(HookMethodAttribute), true).FirstOrDefault() as HookMethodAttribute).GetOriginalMethodName(hookMethod); ;
        ...
        private void InstallInternal(bool isInstall, Assembly[] assemblies)
        {
            foreach (var detour in destAndOris)
            {
                ...
                var hookMethodAttribute = hookMethod.GetCustomAttributes(typeof(HookMethodAttribute), true).FirstOrDefault() as HookMethodAttribute;
                //获取当前程序集中的基础类型
                ...
                if (rawMethod != null && rawMethod.IsGenericMethod)
                {
                    ...
                    rawMethod = ((MethodInfo)rawMethod).MakeGenericMethod(hookMethod.GetParameters().Select(o =>
                    {
                        ...
                        var attr = o.GetCustomAttributes(typeof(RememberTypeAttribute), true).FirstOrDefault() as RememberTypeAttribute;
                ...
                engine = DetourFactory.CreateDetourEngine();
        ...
        public IDetour engine;

        public Type TypeResolver(string typeName, Assembly[] assemblies)
        {
        ...
        //查找匹配函数
        private MethodBase FindMethod(MethodBase[] methods, string name, MethodBase like, Assembly[] assemblies)
        {
            ...
            foreach (var item in methods)
            {
                ...
                for (var i = 0; i < len; i++)
                {
                    ...
                    var rmtype = t1.GetCustomAttributes(typeof(RememberTypeAttribute), true).FirstOrDefault() as RememberTypeAttribute;
                    //手动保持的类型
