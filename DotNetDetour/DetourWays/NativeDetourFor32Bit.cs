...
namespace DotNetDetour.DetourWays
{
    ...
    public unsafe class NativeDetourFor32Bit : IDetour
    {
        ...
        private List<byte> Ori = new List<byte>();
        private IntPtr originalSite;

        public void UnPatch()
        {
            NativeAPI.VirtualProtect(originalSite, 5, Protection.PAGE_EXECUTE_READWRITE, out uint oldProtecton);
            for (int i = 0; i < Ori.Count; i++)
            {
                *(rawMethodPtr + i) = Ori[i];
            }
        }
